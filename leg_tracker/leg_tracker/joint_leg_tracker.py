...
# Custom messages
from leg_tracker_interfaces.msg import Person, PersonArray, Leg, LegArray
...
class ObjectTracked:
    ...
    def __init__(self, x, y, now, confidence, is_person, in_free_space, scan_frequency):
        """
        Constructor
        """
...
class KalmanMultiTracker(Node):
    ...
    def __init__(self):
        ...
        Constructor
        """
        super().__init__("kalman_multi_tracker")
        ...
        self.declare_parameter("max_leg_pairing_distance", 0.8)
        self.declare_parameter("confidence_threshold_to_maintain_track", 0.1)
        self.declare_parameter("publish_occluded", True)
        self.declare_parameter("publish_detected_people", True)
        self.declare_parameter("publish_people_frame", "odom")
        ...
        self.max_leg_pairing_dist = (
            self.get_parameter("max_leg_pairing_distance")
            .get_parameter_value()
            .double_value
        )
        ...
        self.detected_clusters_sub = self.create_subscription(
            LegArray, "detected_leg_clusters", self.detected_clusters_callback, 10
        )
        self.local_map_sub = self.create_subscription(
            OccupancyGrid, "local_map", self.local_map_callback, 10
        )

        self.get_logger().info("Kalman Multi Tracker started!")
            ...
            for person in self.objects_tracked:
                if person.is_person == True:
                    if (
                        ...
                        quaternion = tf2_ros.transformations.quaternion_from_euler(
                            0, 0, yaw
                        )
                        ...
                        marker.color.a = (
                            rclpy.duration.Duration(3)
                            - (self.get_clock().now() - person.last_seen)
                        ).to_sec() / rclpy.duration.Duration(3).to_sec() + 0.1
                        marker.pose.position.x = ps.point.x
                        marker.pose.position.y = ps.point.y
                        marker.id = marker_id
                        marker_id += 1
