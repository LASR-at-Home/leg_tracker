...
from pykalman import (
    KalmanFilter,
)  # Third party library. To install: http://pykalman.github.io/#installation
import numpy as np
from munkres import (
    Munkres,
)  # Third party library. For the minimum matching assignment problem. To install: https://pypi.python.org/pypi/munkres
import random
...
class KalmanMultiTracker:
    ...
    def __init__(self):
        ...
        self.confidence_threshold_to_maintain_track = rospy.get_param(
            "confidence_threshold_to_maintain_track", 0.1
        )
        self.publish_occluded = rospy.get_param("publish_occluded", True)
        self.publish_people_frame = rospy.get_param(
            "publish_people_frame", self.fixed_frame
        )
        self.use_scan_header_stamp_for_tfs = rospy.get_param(
            "use_scan_header_stamp_for_tfs", False
        )
        self.publish_detected_people = rospy.get_param("display_detected_people", False)
        self.dist_travelled_together_to_initiate_leg_pair = rospy.get_param(
        ...
        # ROS publishers
        self.people_tracked_pub = rospy.Publisher(
            "people_tracked", PersonArray, queue_size=300
        )
        self.people_detected_pub = rospy.Publisher(
            "people_detected", PersonArray, queue_size=300
        )
        self.marker_pub = rospy.Publisher(
            "visualization_marker", Marker, queue_size=300
        )

        # ROS subscribers
        self.detected_clusters_sub = rospy.Subscriber(
            "detected_leg_clusters", LegArray, self.detected_clusters_callback
        )
    ...
    def match_detections_to_tracks_global_nearest_neighbour(
        ...
        if match_dist:
            ...
            for elig_detect_index, track_index in indexes:
                if (
                    match_dist[elig_detect_index][track_index]
                    < self.mahalanobis_dist_gate
                ):
    ...
    def detected_clusters_callback(self, detected_clusters_msg):
        ...
        for cluster in detected_clusters_msg.legs:
            detected_clusters.append(
                DetectedCluster(
                    cluster.position.x, cluster.position.y, cluster.confidence
                )
            )
        ...
        for track in propogated:
            track.update(
                np.ma.masked_array(np.array([0, 0]), mask=[1, 1])
            )  # Update copied person with missing measurements

        # Match detected people to existing tracks
        matched_tracks = self.match_detections_to_tracks_global_nearest_neighbour(
        ...
        for detect in detected_clusters:
            if not detect in matched_tracks.values():
                self.objects_tracked.append(
                    ObjectTracked(detect.pos_x, detect.pos_y, now, detect.confidence)
                )
        ...
        for track_1 in self.objects_tracked:
            for track_2 in self.objects_tracked:
                if (
                    ...
                    and not track_1.person
                    and not track_2.person
        ...
        for person in self.people_tracked:
            ...
            dist = (
                (person.leg_1.pos_x - person.leg_2.pos_x) ** 2
                + (person.leg_1.pos_y - person.leg_2.pos_y) ** 2
            ) ** (1.0 / 2.0)
            if (
                dist > 2.0 * self.max_leg_pairing_dist
                ...
                if (
                    person.leg_1.confidence
                    < self.confidence_threshold_to_maintain_track
                    or person.leg_2.confidence
                    < self.confidence_threshold_to_maintain_track
                ):
        ...
        if not transform_available:
            ...
            for person in self.people_tracked:
                ...
                if (
                    ...
                    or leg_1.seen_in_current_scan
                    or leg_2.seen_in_current_scan
                ):
                    # Get person's position in the <self.publish_people_frame> frame
                    ps = PointStamped()
                    ps.header.frame_id = self.fixed_frame
                    ps.header.stamp = tf_time
                    ps.point.x = (leg_1.pos_x + leg_2.pos_x) / 2.0
                    ps.point.y = (leg_1.pos_y + leg_2.pos_y) / 2.0
                    try:
                        ps = self.listener.transformPoint(self.publish_people_frame, ps)
                    except:
                        rospy.logerr(
                    ...
                    marker.color.a = (
                        rospy.Duration(3) - (rospy.get_rostime() - leg_1.last_seen)
